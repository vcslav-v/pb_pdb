...
def create_push(img_url: str, prefix: str):
    ...
    with io.BytesIO(resp.content) as img_file:
        ...
        with io.BytesIO() as buffer:
            img.convert('RGB').save(buffer, format='JPEG')
            url = space_tools.save_to_space(buffer.getvalue(), prefix, 'image_for_push.jpg')
        return url
...
def prepare_imgs(product: schemas.UploadProduct, product_files: schemas.ProductFiles) -> schemas.ProductFiles:
    ...
    if not product_files.push_url:
        new_product_files.push_url = create_push(new_product_files.thumbnail_x2_url, product.prefix)
